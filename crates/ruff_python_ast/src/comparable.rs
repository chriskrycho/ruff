...
//             optional_vars: with_item.optional_vars.as_ref().copied().map(Into::into),
//         }
//     }
// }
//
...
//                     pattern: pattern.map(|pattern| pattern.into()),
//                     name: name.as_ref().map(|name| name.as_str()),
//                 })
//             }
...
// }
//
// impl<'a, 'ast> From<&'a ast::Pattern<'ast>> for Box<ComparablePattern<'a, 'ast>> {
//     fn from(pattern: &'a ast::Pattern<'ast>) -> Self {
...
//             guard: match_case.guard.map(Into::into),
...
//     expression: ComparableExpr<'a, 'ast>,
//     debug_text: Option<&'a ast::DebugText<'ast>>,
...
// }
//
// impl<'a, 'ast> From<&'a ruff_allocator::Box<'ast, ast::Stmt<'ast>>> for ComparableStmt<'a, 'ast> {
//     fn from(stmt: &'a ruff_allocator::Box<'ast, ast::Stmt<'ast>>) -> Self {
//         (&*stmt).into()
//     }
// }
//
